...
vim.api.nvim_create_user_command("SelectedFormatToggle", function(opts)
    ...
    complete = function(_, _, _)
        return {
            ...
            "javascript", "yaml", "html", "css", "scss", "sh", "rust", "go"
        }
    end
})
...
function M.enable_format_on_save(is_configured)
    ...
    vim.api.nvim_create_autocmd("BufWritePre", {
        ...
        callback = function()
            require("modules.completion.formatting").format({
...
function M.format_filter(clients)
    return vim.tbl_filter(function(client)
        ...
        if status_ok and formatting_supported and client.name == "efm" then
            return "efm"
        elseif client.name ~= "sumneko_lua" and client.name ~= "tsserver" and
...
function M.format(opts)
    ...
    for i = 1, #disabled_worksapce do
        if cwd.find(cwd, disabled_worksapce[i]) ~= nil then return end
    end
    ...
    for _, client in pairs(clients) do
        if block_list[vim.bo.filetype] == true then
            vim.notify(string.format("[LSP][%s] format [%s] has disable",
                                     client.name, vim.bo.filetype))
            return
        end
